...
class Position(Base, TimestampMixin):
    ...
    qty: Mapped[float] = mapped_column(Float)  # 剩余持仓量
    initial_qty: Mapped[float] = mapped_column(Float)  # 初始建仓量
    # 多级止盈 [{level:1, price:.., pct:0.3, status:'pending|hit'}]
    tp_levels: Mapped[list] = mapped_column(JSONB, default=list)
    sl: Mapped[float | None] = mapped_column(Float, nullable=True)  # 当前止损价
    ...
    trailing_callback: Mapped[float] = mapped_column(Float, default=0.0)  # 回撤比例
    # status: open(持仓中) / closed(已平仓) / liquidated(强平)
    status: Mapped[str] = mapped_column(String(16), default="open", index=True)
    realized_pnl: Mapped[float] = mapped_column(Float, default=0.0)
    entry_fee: Mapped[float] = mapped_column(Float, default=0.0)  # 开仓手续费(USDT),用于平仓时计算净盈亏
...
class Trade(Base, TimestampMixin):
    ...
    realized_pnl: Mapped[float] = mapped_column(Float, default=0.0)  # 平仓成交时为正负盈亏
