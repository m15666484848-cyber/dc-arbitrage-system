...
async def _cancel_stop(db: AsyncSession, position: Position) -> bool:
    ...
    try:
        ...
        await db.rollback()
        logger.warning(
            f"[交易所止损] 撤单异常 pos={position.id} stop={stop_id}: {str(e)[:200]}"
        )
        return False
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                for pos in positions:
                    if (pos.exchange or "").lower() not in SUPPORTED_EXCHANGES:
                        continue
                    try:
                        await sync_exchange_stop_for_position(db, pos)
                    except Exception as e:
                        logger.warning(
                            f"[交易所止损] 同步异常 pos={pos.id}: {str(e)[:200]}"
                        )
                        try:
                            await db.rollback()
                        except Exception:
