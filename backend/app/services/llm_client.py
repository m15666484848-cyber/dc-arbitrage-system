...
- 严禁编造价格：消息中未明确写出的止损价/止盈价/入场价必须返回 null/空列表，禁止根据历史上下文、前期信号或价格关系推算、编造、凑整、微调任何数字。历史上下文仅用于理解语义，其中出现过的价格不得复制为当前信号参数。所有价格只能原样摘抄当前消息中出现的数字。
...
- 反手指令=平仓+开仓复合动作："空单换手做多/反手做多/反手做空/空翻多/多翻空"表示平掉当前反向持仓并开新仓。必须输出 is_exit_signal=false（不是纯离场）、side=新方向、actions含平仓和开仓两个动作；如"BTC空单市价78800附近成本附近换手做多"=平空+开多。绝不能只输出开仓（会导致旧仓滞留）。
- 条件离场≠立即平仓："如果/若/一旦/要是 + 跌破/突破/回踩/站稳X + 才/就/再考虑 + 离场/出局/出掉/平仓"是价格条件句（价格到达才离场，当前不执行），is_exit_signal=false 且 is_valid_signal=false；如"如果跌破77600我们就考虑离场"=false。只有无条件祈使句（如"所有人多单全部出掉"）才是立即平仓。

更新信号：
- 止损上移/下移/改到/设为/放到/推保护/保本/SL to BE/move SL = is_update_signal=true。
- 改目标/止盈改到/TP 调整 = update_tp_sl。不要解析成新开仓。
- 更新信号中出现的具体价格必须同时填入结构化字段：止损/保护价/保本价后的数字填 stop_loss，止盈目标价填 take_profits；只写在 reasoning 里而字段留空会导致更新被忽略。例如“推保护价：2390(成本价)”= is_update_signal=true 且 stop_loss=2390。
- “打错了/写错了/更正一下/修正一下”是更正上一条信号的参数 = is_update_signal=true，以更正后的价格为准：止损后的数字填 stop_loss，止盈后的数字填 take_profits。例如“止损是15分钟站稳79650 打错了 注意下”= is_update_signal=true 且 stop_loss=79650；缺品种时 symbol=""，由持仓上下文推断，不要因为消息短、口语化或含“仅供参考”就判为无效。
- “止盈X%仓位利润/锁定X%利润/出掉X%仓位”中的百分比是仓位比例（position_pct=X），不是价格；百分比数字严禁填入 take_profits/stop_loss。例如“请止盈70%仓位利润”= position_pct=70，take_profits 必须为空。
...
- "修改入场X/全部修改入场X/改入场价X/统一修改入场X" = update_tp_sl，且 X 必须填入 stop_loss（X是新的保本止损价，不是新开仓入场价，不要填入 entry_price）。
- "成本保护/保本/止损改入场价/止损放开仓价"等未给出具体价格时：is_update_signal=true 且 stop_loss=null，系统会自动按持仓入场价执行，不要编造价格。
- "#X 止损放Y" 格式 = update_tp_sl；尽量从 #X 或上下文提取 symbol，提取不到则 symbol=""。
- "浮盈后提示移动止损/改为开仓价" 若是直接操作提醒，则 update_tp_sl；纯教学文章仍判 none。
- 当日收益汇报 + "全部出局/全平/清仓/空仓休息" = is_exit_signal=true 的平仓信号（如"今日多单收益1500点，空单收益1200点，全部出局空仓休息"），不要因收益汇报字样判为无效。
...
返回严格 JSON，不要 Markdown，不要代码块。"""
    ...
    def _get_kol_parse_hint(self, kol_name: str = "") -> str:
        ...
        if "三马哥" in name:
            hints.append("三马哥常用“睡觉挂单多/空、第1单、挂X”表示限价开仓；消息同时包含挂单/开仓关键词和止盈止损描述时，优先解析为开仓，止盈止损作为订单参数而非平仓动作。")
        if "大镖客" in name:
            hints.append("大镖客的完整策略单含『建仓/止损/止盈』字段；口语补充消息(如『稳健的就挂一个X』)通常只有入场价、无止损止盈，缺失参数返回null，不要编造，系统会自动沿用最近完整策略单参数。")
