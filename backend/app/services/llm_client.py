...
## 方向冲突解决规则（最高优先级）
当一条消息中同时出现中文方向词和英文方向词时，以中文方向词为准：
1. "做多 go long" / "做多Go Long" → side=long（不是 short）
2. "做空 go short" / "做空Go Short" → side=short（不是 long）
3. "做多"出现时，无论后面跟什么英文，方向必须是 long
4. "做空"出现时，无论后面跟什么英文，方向必须是 short
5. "short-term" / "短线" 是时间框架描述，不包含方向信息
6. "stop loss" / "take profit" 是止损止盈，不包含方向信息
7. "反弹阻力空" / "反弹到阻力做空" = 在阻力位做空反弹，方向是 short
...
## 明确信号优先规则（建议语气/杠杆/条件单）
- 只要含明确方向词 + 入场价/价格区间 + 止损或止盈，就是有效交易信号，即使前面有"激进的同学/可以/建议/最好等"等语气修饰。
- "激进的同学开空X附近 止损Y 止盈Z" = open_short；不要因"激进的同学"判为 none。
- "1倍空" / "1倍做空" / "1倍多" / "1倍做多" 是合约信号（1倍杠杆），不是现货或非信号，side 按方向词判定。
- "等跌破X反弹Y开空" / "跌破X以后反弹到Y空" = 条件单，condition_price=X, entry_price=Y, side=short。
- "等站稳X做多" / "如果到X做多" = 条件单，condition_price=X, side=long。
- "反弹"单独出现不等于做多，必须结合"做多/做空/多单/空单/阻力空/支撑多"等上下文判断方向。
