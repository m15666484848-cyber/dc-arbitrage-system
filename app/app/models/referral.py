...
class ReferralCommission(Base, TimestampMixin):
    ...
    # 下级的净盈亏(USDT)
    invitee_pnl: Mapped[float] = mapped_column(default=0.0)
    # 佣金比例(如 0.1 = 10%)
    commission_rate: Mapped[float] = mapped_column(default=0.1)
    # 佣金金额(USDT) = invitee_pnl * commission_rate
    commission_amount: Mapped[float] = mapped_column(default=0.0)
    # 交易品种(冗余,方便查询)
